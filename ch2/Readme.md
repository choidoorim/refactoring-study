...
- 테스트 주도 개발(Test Driven Development): 테스트 코드 + 리팩터링


- 애자일을 제대로 적용하기 위해서는 개발 프로세스 전반에 리팩터링이 자연스럽게 스며들도록 해야한다.


- 리팩터링의 첫 번째 토대는 바로 **테스트 코드**이다. 즉, 프로그래밍 도중 발생한 오류를 확실히 걸러내는 테스트를 자동으로 수행할 수 있어야 한다.


- 지속적 통합(CI)을 통헤 팀원 각자가 수향한 리팩터링 결과를 빠르게 동료와 공유하여 다른 팀원의 작업에 문제를 일으키면 즉시 해결하기 쉽다.


- 테스트 코드, 지속적 통합, 리팩터링이라는 기법은 서로 강력한 상승효과를 발휘한다.

## 2.8 리팩터링과 성능
- **하드 리얼타임 시스템을 제외**한 소프트웨어를 빠르게 만드는 비결은, 먼저 튜닝하기 쉽게 만들고 나서 우너하는 속도가 나게끔 다시 튜닝을 하는 것이다.
